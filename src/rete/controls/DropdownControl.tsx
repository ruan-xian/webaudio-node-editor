...
export class DropdownControl extends Classic.InputControl<"text"> {
  constructor(change: () => void, public menuOptions: DefaultOptionType[], public initial?: string, public label?: string, ) {
    super("text", { 
...
export function CustomDropdownControl(props: { data: DropdownControl }) {
  const defaultVal = props.data.initial ? props.data.initial : props.data.menuOptions[0]?.value?.toString() || "default"
  if (props.data.label) {
    ...
    return (<Select defaultValue={defaultVal} options={props.data.menuOptions} onChange={val => props.data.setValue(val)} style={{ width: 140 }}></Select>);
  }
}